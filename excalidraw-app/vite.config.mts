...
export default defineConfig(({ mode }) => {
  ...
  return {
    server: {
      ...
      open: true,
      watch: {
        // the local storage server writes uploaded scene/library/file blobs
        // into the repo; watching them exhausts the OS inotify limit
        // (ENOSPC) and crashes the dev server, so ignore that data dir.
        ignored: ["**/storage-server/data/**"],
      },
    },
